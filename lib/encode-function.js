...
const byId = (registry) => (schemaId, msg, parseOptions = null) => (() => new Promise((resolve, reject) => {
  ...
  promise = fetchSchema.fetch(registry, schemaId, parseOptions);
...
const bySchema = (type, registry, pushNewSchemas = true) => (topic, schema, msg, parseOptions = null) => (async () => {
  ...
  if (pushNewSchemas) {
    ...
    for(let i = versions.length - 1; i >= 0; i--) {
      const schemaInfo = await fetchSchema.fetchBySubjectAndVersion(registry, subject, versions[i], parseOptions);
